"""
Phase 3 — Train the Docker log root-cause classifier
─────────────────────────────────────────────────────
Run:  python ml/train_debugger.py

This script:
  1. Builds a labelled dataset of Docker log snippets → root cause labels
  2. Trains a TF-IDF + LogisticRegression pipeline
  3. Saves the pipeline to ml/debugger_model.pkl
  4. Prints accuracy and a classification report

After training, set USE_ML_CLASSIFIER=true in the environment to activate it.
"""
...
# Format: (log_snippet, root_cause_label)
# Add more examples from real production logs to improve accuracy.
...
def train():
    ...
    # Save
    out = pathlib.Path("ml/debugger_model.pkl")
